...
-   
-   - **Application layer**: provides data transmission services for specific applications, such as HTTP, DNS and other protocols. The data unit is the report.
- 
-   **传输层**  ：为进程提供通用数据传输服务。由于应用层协议很多，定义通用的传输层协议就可以支持不断增多的应用层协议。运输层包括两种协议：传输控制协议 TCP，提供面向连接、可靠的数据传输服务，数据单位为报文段；用户数据报协议 UDP，提供无连接、尽最大努力的数据传输服务，数据单位为用户数据报。TCP 主要提供完整性服务，UDP 主要提供及时性服务。

- **Transmission layer**: provides general data transmission services for the process. Since there are many application layer protocols, defining common transport layer protocols can support an increasing number of application layer protocols. The transport layer includes two protocols: the transmission control protocol TCP, which provides connection-oriented and reliable data transmission services, and the data unit is the packet segment; the user datagram protocol UDP, which provides data transmission services without connection and doing its best, and the data unit is the user datagram. TCP mainly provides integrity services, and UDP mainly provides timeliness services.
- 
-   **网络层**  ：为主机提供数据传输服务。而传输层协议是为主机中的进程提供数据传输服务。网络层把传输层传递下来的报文段或者用户数据报封装成分组。

- **Network layer**: provide data transmission services for the host. The transport layer protocol provides data transmission services for processes in the host. The network layer encapsulates the packet segment or user data packet transmitted from the transmission layer.
- 
-   **数据链路层**  ：网络层针对的还是主机之间的数据传输服务，而主机之间可以有很多链路，链路层协议就是为同一链路的主机提供数据传输服务。数据链路层把网络层传下来的分组封装成帧。

- **Data link layer**: The network layer is still aimed at data transmission services between hosts, and there can be many links between hosts. The link layer protocol is to provide data transmission services for hosts of the same link. The data link layer encapsulates the packets transmitted from the network layer into frames.

-   **物理层**  ：考虑的是怎样在传输媒体上传输数据比特流，而不是指具体的传输媒体。物理层的作用是尽可能屏蔽传输媒体和通信手段的差异，使数据链路层感觉不到这些差异。

- **Physical layer**: It considers how to transmit data bit streams on the transmission media, not to the specific transmission media. The function of the physical layer is to shield the differences in transmission media and means of communication as much as possible, so that the data link layer does not feel these differences.
